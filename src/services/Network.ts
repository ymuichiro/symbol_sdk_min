...
export class NetworkScripts extends BaseScripts {
  ...
  static async getCurrentType(uri: string) {
    ...
    return { ...res, name: this.getEnumNetworkType(res.data.name) };
  }

  /** get network fee at connected node */
  static async getFees(uri: string) {
  ...
  static async getNetworkProperties(uri: string) {
  ...
  static getEnumNetworkType(type: 'mainnet' | 'testnet'): NetworkType {
    switch (type) {
      ...
      default:
        throw new InternalError(`NetworkType: ${type} is not defined`);
    }
  }
}
