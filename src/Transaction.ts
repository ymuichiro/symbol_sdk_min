...
export class TransactionScripts extends BaseScripts {
  ...
  static calculateFees(message: string, mosaics: Mosaic[], currentNetworkFee: number, epochAdjustment: number, type: NetworkType): number {
    ...
    const transaction = this.createTransferTransaction(dummyAcccount.address.plain(), mosaics, epochAdjustment, type, msgs, currentNetworkFee);
    return transaction.maxFee.compact() / Math.pow(10, 6);
  }

  /** get network fees by selected fee multiplier */
  static async getCurrentNetworkFees(selectedFeeMultiplier: TransactionFeeConfig, networkFees: TransactionFeesDTO, defaultDynamicFeeMultiplier: number): Promise<number> {
    let fee: number = 0;
    if (selectedFeeMultiplier === TransactionFeeConfig.slow) {
      const fees = networkFees.lowestFeeMultiplier < networkFees.minFeeMultiplier ? networkFees.minFeeMultiplier : networkFees.lowestFeeMultiplier;
      fee = fees || defaultDynamicFeeMultiplier;
    } else if (selectedFeeMultiplier === TransactionFeeConfig.normal) {
      const fees = networkFees.medianFeeMultiplier < networkFees.minFeeMultiplier ? networkFees.minFeeMultiplier : networkFees.medianFeeMultiplier;
      fee = fees || defaultDynamicFeeMultiplier;
    } else {
      const fees = networkFees.highestFeeMultiplier < networkFees.minFeeMultiplier ? networkFees.minFeeMultiplier : networkFees.highestFeeMultiplier;
      fee = fees || defaultDynamicFeeMultiplier;
    }
    return fee;
  }
  ...
  static getEncryptMessage(message: string, toPublicKey: string, fromPrivateKey: string, type: NetworkType): EncryptedMessage {
  ...
  static createTransferTransaction(address: string, mosaic: Mosaic[], epochAdjustment: number, type: NetworkType, message: Message, currentNetworkFees?: number): TransferTransaction {
  ...
  /** sign to transaction */
  static sign(privateKey: string, tx: TransferTransaction, type: NetworkType, generationHash: string): SignedTransaction {
    return Account.createFromPrivateKey(privateKey, type).sign(tx, generationHash);
  }

  /** announce to network of transfer transaction */
  static async announceTransferTransaction(uri: string, signedTx: SignedTransaction): Promise<AnnounceTransactionInfoDTO> {
  ...
  static async announcePartialTransaction(uri: string, signedTx: SignedTransaction): Promise<AnnounceTransactionInfoDTO> {
    if (signedTx.type !== TransactionType.AGGREGATE_BONDED) {
      throw new InternalError("An incorrect method was used.");
    }
  ...
  static async getTransactionInfo(uri: string, path: TransactionGroup, transactionIds: string): Promise<TransactionInfoDTO> {
  ...
  static async getTransactionsInfo(uri: string, path: TransactionGroup, transactionIds: string[]): Promise<TransactionInfoDTO[]> {
    const url = new UrlControler(uri).addPath("transactions", path).exec();
    const body = JSON.stringify({ transactionIds });
    return await this.request<TransactionInfoDTO[]>(url, { method: "POST", body }, { timeout: TIME_OUT });
  };
  ...
  /** search transaction */
  static async searchTransactions(uri: string, path: TransactionGroup, criteria: SearchTransactionsRequest): Promise<TransactionPage> {
